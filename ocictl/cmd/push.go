...
func newPushCmd(out io.Writer) *cobra.Command {
	...
	f := cmd.Flags()
	f.StringVarP(&pc.path, "path", "p", "", "Path to your beval.yaml or push.yaml. By default will look in the current working directory")
...
func (p *pushCmd) run(args []string) error {
	...
	go builder.Push(bevalSpec, res, errChan, finished)
